...
//   console.log(results.filter ( item => item.freight < 1 ));

// 2) customerID ERNSH olan neçə ədəd sifariş var?



// let costumeridERNSH = (results.filter(item => item.order.customerId === "ERNSH"));
// console.log(costumeridERNSH.length);


// 3) employeeİd- görə sortlayın


// console.log(results.sort((a,b) => a.order.employeeId-b.order.employeeId));


// 4) OrderDetailsi 3-dən çox olanları console-a yazdır


// console.log(results.filter(item => item.orderDetails.length > 3 ));

// 5) İD-si 11075 olanı tap və yazdır. (find)

console.log(results.filter( item => item.order.id=== 11075));
